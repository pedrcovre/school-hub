...
const Resource = () => {
  ...
  const filteredResources = resources.filter(resource => {
    ...
    if (selectedTab === 'Fechado')
      return resource.Status === 'Aprovado' || resource.Status === 'Recusado'
    return true
  })
    ...
    <>
      <div className='min-h-screen p-10'>
        <div className='flex justify-between mx-[28%] items-center mb-15'>
          ...
          <button
            className='bg-[#5CA4F5] text-white px-10 rounded-xl h-12'
          >
            Nova Requisição
          </button>
        </div>
            ...
            <tbody>
              {currentItems.map((item, index) => (
                ...
                >
                  ...
                  <td className='py-4 text-black/50 w-40'>{item.tipo}</td>
                  <td className='py-4 text-black/50 w-40'>{item.data}</td>
                  <td className='py-4 w-40'>
                    <div className='py-2 rounded-xl bg-gray-200 font-medium'>
                      {item.Status}
                    </div>
                  </td>
                  <td className='py-4 text-black/50 w-40'>
                    {item.Responsavel}
                  </td>
                  <td className='py-4 w-40 text-black/70 font-semibold cursor-pointer'>
                    Ver
                  </td>
                </tr>
              ))}
